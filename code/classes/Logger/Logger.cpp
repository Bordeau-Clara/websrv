...
Logger::~Logger()
{
    ...
    this->_stream << ANSI_CLEAR_LINE << ANSI_MOVE_START  << RESET << std::endl;
}
...
void Logger::_refreshStatusLine(void) const
{
    ...
    this->_stream << ANSI_CLEAR_LINE << ANSI_MOVE_START << strTimestamp() + _status_line << RESET << std::flush;
}
...
void Logger::printNewLine(const String& msg) const
{
    ...
    this->_stream << strTimestamp() + msg << RESET << std::endl;
